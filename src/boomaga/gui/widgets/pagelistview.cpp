...
void PagesListView::updateItems()
{
    ...
    for (int i=0; i<count(); ++i)
    {
        ...
        item->setData(PREVIEWPAGE_NUM_ROLE, Project::instance()->previewPageNum(page.page));
...
void PagesListView::switchPageNum()
{
    ...
    int pageNum = Project::instance()->currentPreviewPage();
...
void PagesListView::previewRedy(QImage image, int pageNum)
{
    for (int i=0; i<count(); ++i)
    {
        if (item(i)->data(PAGE_NUM_ROLE) == pageNum)
        {
            ...
            {
                if (Project::instance()->printer()->grayscale())
...
void PagesListView::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    int sheet = Project::instance()->previewSheets().indexOfPage(page);
    if (sheet < 0)
        return;
...
QIcon PagesListView::createIcon(const QImage &image) const
{
    ...
    if (image.isNull())
    {
        QSizeF size = Project::instance()->printer()->paperSize(UnitPoint);
