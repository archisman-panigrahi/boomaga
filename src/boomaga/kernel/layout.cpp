...
int LayoutNUp::calcSheetCount() const
{
    ...
    return ceil(Project::instance()->pageCount() * 1.0 / pps);
}
...
void LayoutNUp::doFillSheets(QList<Sheet *> *sheets, bool) const
{
    ...
    int i=0;
    while (i < Project::instance()->pageCount())
    {
        Sheet *sheet = new Sheet(pps, sheets->count());

        for (int j=0; j<pps; ++j)
        {
            if (i<Project::instance()->pageCount())
            {
                ProjectPage *page = Project::instance()->page(i);
                sheet->setPage(j, page);
            }
            ++i;
        }

        sheet->setRotation(Project::instance()->rotation());
        sheets->append(sheet);
    }
}
...
{
    Printer *printer = Project::instance()->printer();
...
{
    int result = 0;
    QList<BookletInfo> booklets = split(Project::instance()->pages());
...
void LayoutBooklet::fillSheetsForBook(int bookStart, int bookLength, QList<Sheet *> *sheets) const
{
    ...
    for (int i = 0; i < cnt / 2; i+=2)
    {
        ...
        if (n < bookLength)
        {
            ProjectPage *page = Project::instance()->page(n + bookStart);
...
void LayoutBooklet::fillPreviewSheetsForBook(int bookStart, int bookLength, QList<Sheet *> *sheets) const
{
    ...
    for (int i = -1; i < cnt; i+=2)
    {
        ...
        if (i>-1)
        {
            if (i < bookLength)
            {
                ProjectPage *page = Project::instance()->page(i + bookStart);
                sheet->setPage(0, page);
            }
        }
        ...
        if (i+1<cnt)
        {
            if (i +1 < bookLength)
            {
                ProjectPage *page = Project::instance()->page(i + 1 + bookStart);
                sheet->setPage(1, page);
            }
        }
    }
